...
{background:#ccc}.|_.Key|_.Default|_.Description|
|camel.name|play-camel|Name for the CamelContext object|
{background:#eee}.|broker.name|play-activemq|Name for the ActiveMQ Broker|
|broker.connector|tcp://localhost:61616|host:port that will be exposed by the ActiveMQ Connector|
{background:#eee}.|broker.url|vm:localhost|URL used for instanciating an embedded broker|

h2. Using @Inject

p. You can inject @CamelContext@ into any class by using "JSR299":http://jcp.org/en/jsr/detail?id=299:
...
p. *You now have a fully fonctionning EIP + an embedde JMS Broker right on your Play! application*.
